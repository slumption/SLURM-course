...
\date[17/11/2016] % (optional)
{17th November 2016 / UIS Training}
...
\begin{itemize}
\item{Using MobaXterm, connect to the remote desktop running on \alert{login-gfx2.hpc.cam.ac.uk} on display \alert{66}. The VNC password is \alert{\emph{``trAin99''}}.}
\visible<2->{\begin{description}
...
Now go to \alert{Advanced VNC settings}, tick \alert{Connect through SSH gateway} and enter \alert{login-gfx2.hpc.cam.ac.uk} as the gateway server, with your training account ID as the user. Click OK.\hfill\\\smallskip
