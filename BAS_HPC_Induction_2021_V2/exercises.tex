...
%\caption{\label{fig:mate-session}Opening a terminal}
\end{figure}
\end{minipage} \hfill
\begin{minipage}{0.35\textwidth}
\begin{itemize}
...
\item[(b)]{Examine your personal filesystem quotas with the command \alert{myquota}.}
 \visible<2->{\begin{description}
   \item{You should see a quota on /users, and a quota on /data/hpcdata as you have two homes.}
   \end{description}}
\end{itemize}
\end{frame}
}
...
\begin{frame}{Exercise 3: File transfer}
\begin{itemize}
\item{Use SFTP to transfer the file \alert{exercises.tgz} to your home directory}
\visible<2->{\begin{description}
\item[\emph{Hints:}]{\small The command is \alert{sftp}. Use the same remote host, username and password as in the previous exercise.\\\smallskip
  Use \alert{mkdir exercises} to create a directory then \alert{cd exercises} to change into the target directory, then \alert{put exercises.tgz} to transfer the file from your desktop to the target directory on the cluster. Use \alert{quit} to close the connection.}
...
\visible<2->{\begin{description}
\item[\emph{Hints:}]{Do \alert{ls -al\quad$\tilde{}$/exercises/}. Note that you can often reduce typing by pressing \alert{TAB}.}
\end{description}}
\item{Unpack the tar archive to create an exercise subdirectory.}
\visible<3->{\begin{description}
\item[\emph{Hints:}]{Do \alert{cd\quad$\tilde{}$/exercises/} then \alert{tar -zxvf exercises.tgz}.}
\end{description}}
\end{itemize}
\end{frame}
}    
...
\visible<2->{\begin{description}
\item[\emph{Hints:}]{\small Firstly you may need to review Exercise~1 in order to reconnect to your cluster account. At the remote command prompt, change to the exercises directory (\alert{cd $\tilde{}$/exercises}).}
\end{description}}
